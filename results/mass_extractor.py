...
parser.add_argument(
    "-o", required=False, default="log_summary1.csv", help="output file", type=str
)
...
for folder in folder_list:
    ...
    try:
        ...
        new_df["num_training_images"] = re.findall(
            r"num_training_images_([a-z,0-9,-]+)", folder
        )[0]
        new_df["unfrozen_blocks"] = re.findall(r"unfrozen_blocks_([a-z,0-9]+)", folder)[
            0
        ]
        new_df["seed"] = re.findall(r"seed_([a-z,0-9]+)", folder)[0]
        new_df["downscaled"] = re.findall(r"downscaled_([a-z,A-Z]+)", folder)[0]
